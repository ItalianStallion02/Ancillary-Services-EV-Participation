...
#
# This optimized version removes the big M notation
#
# The only bound (M) is on min/max AS bid size
#
# Rememeber to scale M as fleet order of magnitude / bid upper bound grows
#
...
def solve_stochastic_joint_bidding(
    ...
    for t in range(T):
        ...
        # -------------------------------------------------------------------
        if t < T - 1:
            # A transition only occurs if a block starts or ends at t+1.
            # If no transition occurs, the indicator sum is 0, freezing the profiles perfectly.
    ...
    # Day-ahead invariant baseline energy cost component
...
if __name__ == "__main__":
    solve_stochastic_joint_bidding(
        ...
        C_slack              = 1,
        M                    = 450, # Capped strictly at fleet design capability
        dt                   = 0.25,
        deployment_seed      = 15,
        H                    = 8,   
        min_bid_kw           = 100.0,
