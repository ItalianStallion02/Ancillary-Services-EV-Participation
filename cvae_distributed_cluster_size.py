...
# ==========================================
# 1. THE CVAE-GMM MODEL WITH FLEET-GENERATION
# ==========================================
class EV_DayAhead_CVAE(nn.Module):
    ...
    def generate_day_ahead_fleet_scenarios(self, single_day_c, num_vehicles_N, num_scenarios_S=5):
        self.eval()
        ...
        with torch.no_grad():
            for s in range(num_scenarios_S):
                ...
                chosen_cluster_indices = torch.multinomial(GMM_probabilities, num_vehicles_N, replacement=True)
                
                sampled_fleet_sessions = []
                vehicle_origin_clusters = []
                
                for n in range(num_vehicles_N):
                    k_selected = chosen_cluster_indices[n].item()
                    ...
                    vehicle_origin_clusters.append(k_selected)
...
# ==========================================
# 3. ADVANCED DATA PREPROCESSING LAYER WITH COUNT HISTOGRAMS
# ==========================================
...
if __name__ == "__main__":
    ...
    model = EV_DayAhead_CVAE(target_dim=3, context_dim=5, latent_dim=3, dictionary_dim=10, num_clusters_K=15).to(device)
    ...
    print("\n--- PHASE B: EXECUTING JOINT COUNT-MIXTURE DAY-AHEAD GENERATION ---")
    ...
    NUM_SCENARIOS = 3
    print("\n" + "="*80)
    print("                      PRODUCTION JOINT SCENARIO OUTPUTS                         ")
    print("" + "="*80)
    
    for s in range(NUM_SCENARIOS):
        sampled_day_N = np.random.poisson(poisson_lambda)
        if sampled_day_N < 1: sampled_day_N = 1  
        
        day_scenario = model.generate_day_ahead_fleet_scenarios(tomorrow_context, num_vehicles_N=sampled_day_N, num_scenarios_S=1)[0]
        ...
        ev_counts_per_cluster = np.bincount(fleet_origins, minlength=model.K)
        total_day_energy_request = fleet_real[:, 1].sum().item()
        
        print(f"\n>>>> [JOINT FLEET SCENARIO #{s+1:02d} | Attendance Count: {sampled_day_N} EVs] <<<<")
        ...
        for k in range(model.K):
            ...
            cluster_assigned_evs = ev_counts_per_cluster[k]
            
            # UPDATED: Explicitly pulling c_mu[2] for the Departure time metric visualization
            print(f"    -> Cluster #{k:02d}: Probability {pct_weight:5.1f}% | ASSIGNED VOLUME: {cluster_assigned_evs:2d} EVs | Profile Center: [Arr: {c_mu[0]%24:5.2f}h, Energy: {c_mu[1]:5.2f}kWh, Dep: {c_mu[2]%24:5.2f}h]")